...
export const ContenuFormulaire = () => {
    ...
    useEffect(()=>{
        if(userId<0){
            navigate(`/login`)
          }else if(nameClass!== USER_TYPES.USER_FORMATEUR){
            switch(nameClass){
              case USER_TYPES.USER_ETUDIANT : { navigate(`/etudiant`) }
              case USER_TYPES.USER_MANAGER : { navigate(`/manager`) }
  ...
  return (
    <main className='my-10 container mx-auto min-h-screen text-center'>
        ...
        <form className='sm:w-full lg:w-2/3 mx-auto flex flex-col gap-2' onSubmit={handleSubmit} >
            ...
            <BtnSubmit className={"bg-pink-50 text-pink-700 hover:bg-pink-300 py-3 px-7 font-bold rounded-sm my-6 shadow-sm shadow-black"} text={contenuId>0 ? "Modifier" : "Créer"} />
                </form> 
    </main>
  )
}
